...
# Fonction pour calculer la similarité de Monge-Elkan entre deux ensembles
def monge_elkan_similarity(set1, set2):
    if not set1 or not set2:  # Vérifie si l'un des ensembles est vide
        return 0.0
    
    # Calculer la similarité entre chaque élément de set1 et set2
    similarities = []
    for elem1 in set1:
        max_sim = max(fuzz.ratio(elem1, elem2) for elem2 in set2)
        similarities.append(max_sim)
    
    return np.mean(similarities) / 100.0  # Normaliser en divisant par 100
...
# Calcul de la matrice de similarité de Monge-Elkan
similarity_matrix = []
for u_elem in uml_activity_elements:
    row = []
    for b_elem in bpmn_elements:
        similarity = monge_elkan_similarity(set(uml_activity_elements[u_elem]), set(bpmn_elements[b_elem]))
