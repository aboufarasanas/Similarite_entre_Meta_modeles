...
# Fonction pour calculer la similarité Cosinus entre deux listes de caractéristiques
def cosine_similarity_features(set1, set2):
    if not set1 or not set2:  # Vérifie si l'un des ensembles est vide
        return 0.0
    
    vectorizer = CountVectorizer().fit_transform([set1, set2])
    vectors = vectorizer.toarray()
    return cosine_similarity(vectors)[0, 1]

# Convertir les éléments en chaînes de caractères pour la vectorisation
...
for u_elem in uml_activity_elements:
    ...
    for b_elem in bpmn_elements:
        similarity = cosine_similarity_features(convert_to_string(uml_activity_elements[u_elem]), 
