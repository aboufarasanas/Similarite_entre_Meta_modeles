...
# Créer une matrice de similarité de Jaro-Winkler
similarity_matrix = np.zeros((len(relational_database_elements), len(key_value_store_elements)))
for i, (rel_name, _) in enumerate(relational_database_elements.items()):
    for j, (kv_name, _) in enumerate(key_value_store_elements.items()):
        similarity_matrix[i, j] = jaro_winkler_similarity(rel_name, kv_name)
